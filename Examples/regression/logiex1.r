...
cas.dataStep.runCode(s,
   ...
   data getStarted;
      ...
      do i=1 to nObsPerThread;
         ...
         x5=round(100*rand(\'uniform\'));
      x6=round(rand(\'normal\')*.8+1.5,.1);
...
cas.regression.logistic(s,
    class=c('C'),
    model=list(depvar='y',
               effects=c('C', 'x1', 'x2', 'x3', 'x4', 'x5', 'x6', 'x7', 'x8',
                         'x9', 'x10')),
    optimization=list(itHist='summary'),
    outputTables=list(names=list(parameterestimates='pe')),
    table='getStarted')

cas.regression.logistic(s,
    class=c('C'),
    display=list(traceNames='true'),
    model=list(depvar='y',
               effects=c('C', 'x1', 'x2', 'x3', 'x4', 'x5', 'x6', 'x7', 'x8',
                         'x9', 'x10')),
    selection=list(details='all', method='forward'),
    table='getStarted')
